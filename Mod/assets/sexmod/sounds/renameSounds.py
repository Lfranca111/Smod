...
for file in os.listdir():
	...
	if(file.endswith('.mp3')):
		...
		AudioSegment.from_file(file, "mp4").export(soundName+str(i)+'.ogg', format='ogg')
		print("turned "+ file + " into " + soundName+str(i)+'.ogg')
		os.remove(file)

	elif(file.endswith('.wav')):
		AudioSegment.from_file(file, "wav").export(soundName+str(i)+'.ogg', format='ogg')
		print("turned "+ file + " into " + soundName+str(i)+'.ogg')
		os.remove(file)

	elif(file.endswith('.ogg')):

		os.rename(file, soundName+str(i)+'.ogg')
		print("turned "+ file + " into " + soundName+str(i)+'.ogg')
		#os.remove(file)
